...
def load_nwchem_defaults(options):
    ...
    options.add(
        'nwchem',
        RottenOption(keyword='dft__semidirect__memsize', default='', validator=parsers.parse_memory, glossary=''))

    options.add(
        'nwchem',
        RottenOption(keyword='dft__semidirect__filename', default='', validator=lambda x: x.lower(), glosary=''))
    ...
    options.add(
        ...
        RottenOption(
            ...
            default='',
            validator=parsers.enum(
                "LCCD CCD LCCSD CCSD CCSD_ACT LR-CCSD EACCSD IPCCSD CC2 CCSDT CCSDTA CCSDTQ CCSDTQ CCSD(T) CCSD[T] CR-CCSD[T] CR-CCSD(T) CCSD(2)_T CCSD(2)_TQ CCSDT(2)_Q LR-CCSD(T) LR-CCSD(TQ)-1 CREOMSD(T) CREOM(T)AC QCISD CISD CISDT CISDTQ MBPT2 MBPT3 MBPT4 MP2 MP3 MP4"
            ),
    ...
    options.add(
        'nwchem',
        RottenOption(
            keyword='tce_maxiter', default=100, validator=parsers.positive_integer, glosssary='TCE max iterations'))
    ...
    options.add(
        'nwchem',
        RottenOption(keyword='tce_2emet', default=1, validator=parsers.positive_integer, glossary='Default is 1.'))
    ...
    options.add('nwchem',
                RottenOption(keyword='tce_tilesize', default='', validator=parsers.positive_integer, glossary=''))
    ...
    options.add(
        ...
        RottenOption(
            keyword=
            'task_tce_mp2',  #not sure if need here but need to distinguish between MP2/MP3/MP4 and TCE MBn theory
            default='energy',
            validator=lambda x: x.upper(),
