...
def find_derivative_type(ptype, method_name, user_dertype, user_package):
    ...
    if user_dertype is None:
        ...
        if not isinstance(user_dertype, int):
            raise ValidationError(
                "find_derivative_type: user_dertype ({}) should only be None or int!".format(user_dertype)
            )
    ...
    if (dertype == 2) and (method_name in procedures["hessian"][package]):
        ...
        raise ValidationError(
            """Derivative method 'name' ({}) and derivative level 'dertype' ({}) are not available.{}""".format(
                method_name, dertype, alternatives
            )
        )
