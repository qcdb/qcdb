...
def load_nwchem_keywords(options: Keywords) -> None:
    ...
    options.add(
        "nwchem",
        Keyword(
            keyword="property__octupole", default=False, validator=parsers.boolean, glossary="Determine octupole moment"
        ),
    )
    ...
    options.add(
        "nwchem",
        Keyword(
            keyword="property__efield", default=False, validator=parsers.boolean, glossary="Electric field at nuclei"
        ),
    )
    ...
    options.add(
        "nwchem", Keyword(keyword="property__gshift", default=False, validator=parsers.boolean, glossary="Gshift")
    )  # nwc 6.8 option
    ...
    options.add(
        "nwchem", Keyword(keyword="property__shielding", default="", validator=parsers.atompair, glossary="Shielding")
    )
    ...
    options.add(
        "nwchem",
        Keyword(
            keyword="scf__maxiter", default=20, validator=parsers.nonnegative_integer, glossary="Max SCF iteration"
        ),
    )
    ...
    options.add(
        "nwchem",
        Keyword(
            keyword="scf__direct", default=False, validator=parsers.boolean, glossary="SCF Direct calculation on/off"
        ),
    )
    ...
    options.add(
        "nwchem",
        Keyword(keyword="scf__nr", default=0.1, validator=lambda x: float(x), glossary="Control Newton-Raphson value."),
    )
    ...
    options.add(
        "nwchem",
        Keyword(
            keyword="driver__noxyz", default=False, validator=parsers.boolean, glossary="No printing of xyz geometries."
        ),
    )
    ...
    options.add(
        ...
        Keyword(
            ...
            default=True,
            validator=parsers.boolean,
            glossary="Default is on. Requests the TDDFT calculation of singlet excited states when reference wave function is closed.",
        ),
    )
    options.add(
        "nwchem",
    ...
    options.add(
        "nwchem",
        Keyword(keyword="mp2__fss", default=1.2, validator=lambda x: float(x), glossary="Scaling factor for same spin"),
    )

    options.add(
        "nwchem",
        Keyword(
            keyword="mp2__fos", default=0.3, validator=lambda x: float(x), glossary="Scaling factor for opposite spin"
        ),
    )
    ...
    options.add(
        "nwchem",
        Keyword(
            keyword="dft__iterations", default=30, validator=parsers.positive_integer, glossary="Specify DFT iterations"
        ),
    )
    ...
    # DFT XC [Functionals]
    options.add(
        "nwchem", Keyword(keyword="dft__xc", default="", validator=lambda x: x.lower(), glossary="DFT functional")
    )
    ...
    options.add(
        "nwchem",
        Keyword(keyword="tce__maxiter", default=100, validator=parsers.positive_integer, glossary="TCE max iterations"),
    )
    ...
    options.add(
        "nwchem", Keyword(keyword="tce__2emet", default=1, validator=parsers.positive_integer, glossary="Default is 1.")
    )
    ...
    options.add(
        ...
        Keyword(
            ...
            default="medium",
            validator=parsers.enum("debug high medium low none"),
            glossary="TCE level print options which can be divided from most specific to least: debug, high, low, and none. Default is medium.",
        ),
    )
    ...
    options.add(
        "nwchem",
        Keyword(
            keyword="hessian__thresh",
            default=1.0e-6,
            validator=parsers.parse_convergence,
            glossary="Wavefunction convergence for analytic Hessians.",
        ),
    )
