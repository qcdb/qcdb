...
Psi4 mode: When `psi4` the python module is importable at `import qcdb`
           ...
           * qcvar "DISPERSION CORRECTION ENERGY" is set
...
def run_dftd3(molrec, functional=None, dashlevel=None, dashparams=None, dertype=None, verbose=1):
    """
    ...
    """
    jobrec = from_arrays_qc(functional=functional, dashlevel=dashlevel, dashparams=dashparams, dertype=dertype)
...
def from_arrays_qc(functional=None, dashlevel=None, dashparams=None, dertype=None, verbose=1):
    ...
    processed = _validate_and_fill_dashparam(functional=functional, dashlevel=dashlevel, dashparams=dashparams)
    update_with_error(jobrec, processed)
...
def dftd3_plant(jobrec):
    ...
    try:
        ...
        raise KeyError("Required fields missing from ({})".format(jobrec.keys())) from err
    ...
    dftd3par = dashparam.dftd3_coeff_formatter(jobrec["dashlevel"], jobrec["dashparams"])
    dftd3rec["dftd3par"] = dftd3par

    dftd3_geometry = qcel.molparse.to_string(jobrec["molecule"], dtype="xyz", units="Angstrom", ghost_format="")
...
def dftd3_subprocess(dftd3rec):
    ...
    try:
        ...
        raise KeyError("Required fields missing from ({})".format(dftd3rec.keys())) from err
...
def dftd3_harvest(jobrec, dftd3rec):
    ...
    if "dftd3_gradient" in dftd3rec:
        ...
        realgrad = np.fromstring(srealgrad, count=3 * rnat, sep=" ").reshape((-1, 3))
    ...
    formal = {"d2p4": "d2", "d2gr": "d2", "d3zero": "d3", "d3bj": "d3(bj)", "d3mzero": "d3m", "d3mbj": "d3m(bj)"}
...
def _validate_and_fill_dertype(dertype=None):
    ...
    if dertype is None:
        ...
        raise ValidationError("""Requested derivative level ({}) not valid for run_dftd3.""".format(dertype))
...
def _validate_and_fill_dashparam(dashlevel, functional=None, dashparams=None):
    ...
    if functional is None and dashparams is None:
        raise ValidationError(
            """Can't guess -D parameters without functional ({}) or dashparams ({})""".format(functional, dashparams)
        )

    if functional is None:
        dftd3_params = {}
    else:
        functionaleff, dashleveleff, dftd3_params = dashparam.dash_server(functional, dashlevel, return_triplet=True)

    if dashparams is None:
        dashparams = {}
    else:
        dashleveleff, allowed_params = dashparam.dash_server(func=None, dashlvl=dashlevel, return_levelkeys=True)
        if not set(dashparams.keys()).issubset(allowed_params):
            # pass
            raise ValidationError(
                "Requested keys ({}) not among allowed ({}) for dispersion level ({})".format(
                    dashparams.keys(), allowed_params, dashlevel
                )
            )
    ...
    if functional is not None and (
        dftd3_params == dashparam.dashcoeff[dashleveleff][functionaleff]
        or trial == dashparam.dashcoeff[dashleveleff][functionaleff]
    ):
    ...
    return {"dashlevel": dashleveleff, "dashparams": dftd3_params, "functional": functionaleff}
