...
formal_alias = {"d2p4": "d2", "d2gr": "d2", "d3zero": "d3", "d3bj": "d3(bj)", "d3mzero": "d3m", "d3mbj": "d3m(bj)"}
...
dashcoeff = {
    "d2p4": {
        ...
        "dsd-blyp": {
            "s6": 0.35,
            "alpha6": 20.0,
        },  # IN but different btwn dftd3 and psi4 and variants need to be worked out
...
def dash_server(func, dashlvl, return_levelkeys=False, return_triplet=False):
    ...
    if dashlvleff not in dashcoeff.keys():
        raise ValidationError("""Requested -D correction level ({}) not among ({})""".format(dashlvl, dashcoeff.keys()))
...
def dftd3_coeff_formatter(dashlvl, dashcoeff):
    ...
    if dashlvleff.lower() == "d2p4":
        returnstring = dashformatter.format(dashcoeff["s6"], 1.1, 0.0, 0.0, 20.0, 2)
    elif dashlvleff.lower() == "d2gr":
        returnstring = dashformatter.format(dashcoeff["s6"], 1.1, 0.0, 0.0, dashcoeff["alpha6"], 2)
    elif dashlvleff.lower() == "d3zero":
        returnstring = dashformatter.format(
            dashcoeff["s6"], dashcoeff["sr6"], dashcoeff["s8"], 1.0, dashcoeff["alpha6"], 3
        )
    elif dashlvleff.lower() == "d3bj":
        returnstring = dashformatter.format(dashcoeff["s6"], dashcoeff["a1"], dashcoeff["s8"], dashcoeff["a2"], 0.0, 4)
    elif dashlvleff.lower() == "d3mzero":
        returnstring = dashformatter.format(
            dashcoeff["s6"], dashcoeff["sr6"], dashcoeff["s8"], dashcoeff["beta"], 14.0, 5
        )
    elif dashlvleff.lower() == "d3mbj":
        returnstring = dashformatter.format(dashcoeff["s6"], dashcoeff["a1"], dashcoeff["s8"], dashcoeff["a2"], 0.0, 6)
    else:
