...
def define_transition_prop_qcvars(mtd, i, j, extra=""):
    ...
    qcvardefs[f"{mtd} ROOT {i} -> ROOT {j} OSCILLATOR STRENGTH (LEN)"] = {
        "units": "",
        "glossary": """The oscillator strength in length or velocity gauge of named method
...
def define_spin_qcvars(mtd, description, do_spin=False, do_grad=False, extra=""):
    ...
    if do_spin and f"{mtd} SAME-SPIN CORRELATION ENERGY" not in qcvardefs:
        qcvardefs[f"{mtd} SAME-SPIN CORRELATION ENERGY"] = {
            "units": "Eh",
            "glossary": rf"""The unscaled portion of the {mtd} correlation energy from same-spin or triplet doubles correlations.""",
        }

    if do_spin and f"{mtd} OPPOSITE-SPIN CORRELATION ENERGY" not in qcvardefs:
        qcvardefs[f"{mtd} OPPOSITE-SPIN CORRELATION ENERGY"] = {
            "units": "Eh",
            "glossary": rf"""The unscaled portion of the {mtd} correlation energy from opposite-spin or singlet doubles correlations.""",
        }

    if do_spin and f"{mtd} SINGLES ENERGY" not in qcvardefs:
        qcvardefs[f"{mtd} SINGLES ENERGY"] = {
            "units": "Eh",
            "glossary": rf"""The singles portion of the {mtd} correlation energy. Zero except in ROHF.""",
        }
...
def define_ex_transition_qcvars(max_root, mtd, sym):
    ...
    for i in range(max_root):
        qcvardefs["EOM-{} ROOT 0 -> ROOT {} EXCITATION ENERGY - {} SYMMETRY".format(mtd, i + 1, sym)] = {
            "units": "Eh",
            "glossary": """
                The excitation energy of EOM-{} in {} symmetry from 0 to root {}""".format(
                mtd, sym, i
            ),
        }
    ...
    for i in range(max_root):
        qcvardefs["EOM-{} ROOT 0 -> ROOT {} TOTAL ENERGY - {} SYMMETRY".format(mtd, i + 1, sym)] = {
            "units": "Eh",
            "glossary": """
                The total energy of EOM-{} in {} symmetry from 0 to root {}""".format(
                mtd, sym, i
            ),
        }
    ...
    for i in range(max_root):
        for sym in syms:
            qcvardefs[f"TDDFT ROOT {i + 1} EXCITATION ENERGY - {sym} SYMMETRY"] = {
                "units": "Eh",
                "glossary": """ The excitation energy of time-dependent DFT in {} symmetry from 0 to root {}""".format(
                    sym, i + 1
                ),
            }
            ...
            qcvardefs[f"TDDFT ROOT {i + 1} EXCITED STATE ENERGY - {sym} SYMMETRY"] = {
                "units": "Eh",
                "glossary": """The excited state energy of time dependent DFT from root 0 to root {} in {} symmetry""".format(
                    i + 1, sym
                ),
            }
...
qcvardefs["N BASIS FUNCTIONS"] = {"units": "", "glossary": r"""The number of basis functions"""}

qcvardefs["N MOLECULAR ORBITALS"] = {"units": "", "glossary": r"""The number of molecular orbitals"""}
...
define_scf_qcvars(
    "SCF", is_dft=False, extra=" This is a generic HF/DFT quantity and not necessarily aligned across different calcs."
)
...
define_spin_qcvars(
    "MP4(SDQ)",
    description="4rd-order Moller--Plesset perturbation theory without triples excitations",
    do_spin=False,
    do_grad=False,
)
define_spin_qcvars(
    "MP4(SDTQ)", description="4rd-order Moller--Plesset perturbation theory", do_spin=False, do_grad=False
)
define_spin_qcvars(
    "CCSD+T(CCSD)",
    description="coupled cluster singles and doubles with triples evaluated at converged CCSD amplitudes",
    do_spin=False,
    do_grad=False,
)
define_spin_qcvars(
    "CCSDT", description="coupled cluster singles, doubles, and triples excitations", do_spin=True, do_grad=True
)
define_spin_qcvars(
    "CCSDT-1A",
    description="coupled cluster singles, doubles, and triples excitations at approximation 1a",
    do_spin=False,
    do_grad=True,
)
define_spin_qcvars(
    "CCSDT-1B",
    description="coupled cluster singles, doubles, and triples excitations at approximation 1b",
    do_spin=False,
    do_grad=True,
)
define_spin_qcvars(
    "CCSDT-2",
    description="coupled cluster singles, doubles, and triples excitations at approximation 2",
    do_spin=False,
    do_grad=True,
)
define_spin_qcvars(
    "CCSDT-3",
    description="coupled cluster singles, doubles, and triples excitations at approximation 3",
    do_spin=False,
    do_grad=True,
)
define_spin_qcvars(
    "A-CCSD(T)",
    description="coupled cluster singles, doubles, and asymmetric perturbative triples excitations. Also known as Lambda-CCSD(T) or CCSD(AT).",
    do_spin=False,
    do_grad=True,
)
define_spin_qcvars(
...
define_spin_qcvars(
    "QCISD", description="quadratic configuration interaction singles and doubles", do_spin=True, do_grad=False
)
define_spin_qcvars(
    "QCISD(T)",
    description="quadratic configuration interaction singles and doubles with perturbative triples",
    do_spin=False,
    do_grad=False,
)
